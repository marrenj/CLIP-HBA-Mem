...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attention_weights=True):
        attention_weights = []  # To store attention weights if needed
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_token=False, pos_embedding=False):
        ...
        for layer in self.transformer.resblocks:
            x, attn_weights = layer(x)  # Get the output and attention weights from the layer
            layer_outputs.append(x.permute(1, 0, 2))  # Convert back to [batch_size, seq_len, embed_dim]
            attention_weights.append(attn_weights)  # Save attention weights for this layer

        attention_weights = torch.stack(attention_weights, dim=1)  # Convert list to tensor
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)
        x = self.transformer(x, return_attention_weights=False)
        x = x.permute(1, 0, 2)
...
def normalize_weights(weights, method="minmax", **kwargs):
    ...
    if method == "softmax":
        ...
        normalized_weights = (weights - min_val) / (max_val - min_val + 1e-8)
        normalized_weights = normalized_weights / normalized_weights.sum(dim=-1, keepdim=True)  # Ensure sum is 1
...
class ModifiedCLIP(CLIP):
    ...
    def forward(self, image, tokenized_prompts, pos_embedding=False):
        ...
        _, layer_outputs, attention_weights = self.encode_image(image, pos_embedding)
        ...
        return pred_emb_3d, pred_rdm_3d, image_feature_3d, attention_weights
