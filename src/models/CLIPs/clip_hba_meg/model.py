...
class CLIP(nn.Module):
    ...
    def forward(self, image, tokenized_prompts, time_input, pos_embedding=False):
        ...
        image_features = self.time_distortion(
            image_features, time_input
        )  # Apply time distortion to image features
...
def normalize_weights(weights, method="minmax", **kwargs):
    ...
    if method == "softmax":
        ...
        normalized_weights = normalized_weights / (
            normalized_weights.sum(dim=-1, keepdim=True) + 1e-10
        )  # Ensure sum is 1
    elif method == "sum1":
...
class ModifiedCLIP(CLIP):
    def __init__(self, *args, **kwargs):
        ...
        self.weighting_matrix = nn.Parameter(
            torch.tensor(self.weighting_matrix, dtype=torch.float32)
        )
    ...
    def forward(self, image, tokenized_prompts, pos_embedding=False):
        assert self.beta.shape[0] == self.n_timepoints, (
            f"beta scaler shape: {self.beta.shape[0]}, n_timepoints: {self.n_timepoints}"
        )
        ...
        assert len(sample_timepoints) == self.n_train_timepoints, (
            f"Sample timepoints: {len(sample_timepoints)}, n_train_timepoints: {self.n_train_timepoints}"
        )
        ...
        for i in range(self.n_train_timepoints):
            time_input = torch.tensor(
                [i * self.train_step + self.train_start] * batch_size, device=image.device
            )
            ...
            end_index = min(
                int(time_index + self.train_window_size // self.ms_step + 1), self.n_timepoints
            )
            ...
            for j in range(len(layer_outputs)):
                image_features += (layer_outputs[j][:, 0, :] @ self.visual.proj) * (
                    weights[:, j].unsqueeze(1) * visual_scaler
                )
            ...
            noise = torch.randn(batch_size, self.n_dim, device=image.device) * raw_emb.std(
                dim=0, keepdim=True
            )
            pred_emb = F.relu(beta_scaler * (raw_emb + noise * noise_level)) + (
                torch.rand(batch_size, self.n_dim, device=image.device) * 1e-5 + 1e-6
            ).to(image.device)
...
def build_model(
    ...
    if beta is None:
        beta = torch.ones(
            (ms_end - ms_start) // ms_step + 1
        )  # Default to ones if beta generalization is not provided
