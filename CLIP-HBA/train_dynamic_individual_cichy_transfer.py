...
def train_model(model, train_loader, test_loader, device, criterion, p_weight, m_weight, g_weight, optimizer_0, optimizer_1, epochs, fw_tuning_epochs, rdms, sample_timepoints, ms_start, ms_end, ms_step, window_size, early_stopping_patience=5, checkpoint_path='clip_hba_model_cv.pth'):
    ...
    # if best_pearson_loss > initial_pearson_loss:
    #     print("\n\n\n\n<><><><><><><><><><><><><><><><><>")
    #     print(f"Best Pearson Loss: {best_pearson_loss:.4f} is worse than initial Pearson Loss: {initial_pearson_loss:.4f}")
    #     print("<><><><><><><><><><><><><><><><><>\n\n\n\n")
...
def load_pretrained_weights_with_adjustments(model, pretrained_path, train_start, train_end, train_step, pre_trained_fw_matrix=False):
    ...
    for key in list(adjusted_state_dict.keys()):
        ...
        # if pre_trained_fw_matrix == False:
        #     if key == "clip_model.weighting_matrix":
        #         # Interpolate weighting_matrix to match the new shape
        #         del adjusted_state_dict[key]
        # elif key == "clip_model.weighting_matrix":
        #     # Interpolate weighting_matrix to match the new shape
        #     old_weighting_matrix = adjusted_state_dict[key].cpu().numpy()  # Shape: (281, 24)
            
        #     # Interpolation for each column independently
        #     interpolator = interp1d(old_timepoints, old_weighting_matrix, axis=0, kind='linear', fill_value="extrapolate")
        #     interpolated_weighting_matrix = interpolator(new_timepoints)
            
        #     adjusted_state_dict[key] = torch.tensor(interpolated_weighting_matrix, dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    if os.path.exists(model_save_folder):
        ...
        shutil.rmtree(model_save_folder)  # Safely remove the directory and its contents

    os.makedirs(model_save_folder)  # Create the new directory
    ...
    for p_id in range(15):
        ...
        epochs = 500
        fw_tuning_epochs = 5 # None if not specified
        early_stopping_patience = 10
        batch_size = 20
        train_portion = 80/100
        smoothen_window = 5
        ms_start, ms_end, ms_step = -100, 1000, smoothen_window
        train_start, train_end, train_step, train_window_size = -100, 1000, smoothen_window, 15 # (start, end, step) (total 281 timepoints from -100ms to 1300ms every 5ms) #this is the time window for neural finetuning
        lr_1 = 0
        lr_2 = 6e-6
        fw_lr_1= 3e-2
        fw_lr_2 = 1e-2
        checkpoint_path = f'{model_save_folder}/cliphba_dynamic_individual_cichy_p{p_id+1}.pth'
        random_seed = 1
        vision_layers = 24
        transormer_layers = 1
        rank = 32
        ...
        dataset = DynamicDataset_p(csv_file=csv_file, img_dir=img_dir, rdm_dir=rdm_dir, p_id=p_id, smoothen_window=smoothen_window)
        rdms = dataset.rdms
        ...
        plt.legend()
        plt.savefig(f'./curves/individual_curves_p{p_id}.png')
        ...
        # Initialize model
        model = CLIPHBA(classnames=classnames, weighting_matrix=None, backbone_name=backbone, pos_embedding=pos_embedding, ms_start=ms_start, ms_step=ms_step, ms_end=ms_end, train_start=train_start, train_step=train_step, train_end=train_end, train_window_size=train_window_size, beta=beta, noise_level=noise_level, visual_scaler=alpha)
        # print(model)
        ...
        if transfer_learning:
            try:
                load_pretrained_weights_with_adjustments(
                    model, pretrained_path=model_path, train_start=train_start, train_end=train_end, train_step=train_step, pre_trained_fw_matrix=pre_trained_fw_matrix
                )
                print("Things Pretrained weights loaded")
            except Exception as e:
                print(f"Error loading weights: {e}")
                # raise error and stop
                raise ValueError("Error loading pretrained weights")

            if freeze_text:
                freeze_text_encoder(model)
                
        else: 
            print("Not using Things pretrained weights")
        
        if pretrained_text_encoder and not transfer_learning:
        ...
        if pre_trained_fw_matrix == True:
            # Load the weighting matrix as a numpy array and explicitly convert to a PyTorch tensor
            weighting_matrix = torch.tensor(np.load(weighting_matrix_path), dtype=torch.float32)
            
            # Optionally smooth the weighting matrix
            if smoothen_window > 1:
                weighting_matrix = smoothen_weighting_matrix(weighting_matrix, window=smoothen_window)
            
            # Ensure weighting_matrix is a tensor and wrap it in a torch.nn.Parameter
            weighting_matrix = torch.tensor(weighting_matrix, dtype=torch.float32)  # Ensure it's a tensor
            model.clip_model.weighting_matrix = torch.nn.Parameter(weighting_matrix)
            # print(weighting_matrix.max(), weighting_matrix.min(), weighting_matrix.mean())
            print("Pre-optimized Group Weighting matrix loaded successfully\n")
        ...
        optimizer_0 = AdamW([
            {'params': [p for n, p in model.named_parameters() if n != 'clip_model.weighting_matrix'], 'lr': lr_1},  # Default learning rate
            {'params': [model.clip_model.weighting_matrix], 'lr': fw_lr_1}  # Higher learning rate for weighting_matrix
        ])

        optimizer_1 = AdamW([
            {'params': [p for n, p in model.named_parameters() if n != 'clip_model.weighting_matrix'], 'lr': lr_2},  # Default learning rate
            {'params': [model.clip_model.weighting_matrix], 'lr': fw_lr_2}  # Higher learning rate for weighting_matrix
        ])
