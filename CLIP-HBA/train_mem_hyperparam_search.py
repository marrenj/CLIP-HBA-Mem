...
# ---------------------------------------------------------------------------
TRAINING_DATA: str = os.environ.get(
...
if TRAINING_DATA == "lamem":
    ...
    _EMBEDDINGS_DIR: "str | None" = os.environ.get("EMBEDDINGS_DIR") or (
...
SWEEP_DIR: str = (
    f"./sweep_out/{BASE_CONFIG['model_type']}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
)
...
def _objective(
    ...
    try:
        for fold in range(1, N_FOLDS + 1):
            ...
            sweep_stdout.write(f"  fold {fold}/{N_FOLDS}: mse={fold_mse:.6f}  rho={fold_rho:.4f}\n")
            sweep_stdout.flush()
...
def _write_sweep_config(sweep_dir: str, n_trials: int, resumed: bool) -> None:
    ...
    base_config_serialisable = {
        k: v for k, v in BASE_CONFIG.items() if isinstance(v, (str, int, float, bool, type(None)))
    }
    ...
    if not resumed:
        config: dict = {
            ...
            "search_space": {
                k: list(v) if isinstance(v, tuple) else v for k, v in SEARCH_SPACE.items()
            },
...
def run_sweep(n_trials: int = N_TRIALS_DEFAULT, resume_db: str | None = None) -> None:
    ...
    study = optuna.create_study(
        study_name=f"{BASE_CONFIG['model_type']}_bayesian_sweep",
        direction="minimize",
