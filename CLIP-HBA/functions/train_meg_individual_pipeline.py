...
def smoothen_weighting_matrix(weighting_matrix, window=5):
    ...
    reshaped_array = remaining_weighting_matrix[: num_groups * window].reshape(
        num_groups, window, 24
    )
...
def train_model(
    ...
    ms_step,
    window_size,
    early_stopping_patience_0,
    early_stopping_patience_1,
...
def load_pretrained_weights_with_adjustments(
    model, pretrained_path, train_start, train_end, train_step, pre_trained_fw_matrix=False
):
    """
    ...
    np.arange(-100, 1301, 5)
    np.arange(train_start, train_end + 1, train_step)

    # Handle mismatched parameters
    for key in list(adjusted_state_dict.keys()):
        if key in [
            "clip_model.beta",
            "clip_model.noise_level",
            "clip_model.visual_scaler",
            "clip_model.weighting_matrix",
        ]:
...
def run_individual_training(config):
    ...
    for p_id in range(config["n_participants"]):
        ...
        # Train model
        checkpoint_path = (
            f"{config['model_save_folder']}/cliphba_dynamic_individual_cichy_p{p_id + 1}.pth"
        )
        train_model(
            model,
            train_loader,
