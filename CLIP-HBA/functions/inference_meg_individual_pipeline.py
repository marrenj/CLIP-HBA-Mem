...
def run_image(
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_image_names, batch_images) in progress_bar:
            ...
            pred_features = (
                pred_features.cpu().numpy()
            )  # shape: (n_inference_timepoints, batch_size, 768)
            ...
            visual_features[:, current_idx : current_idx + batch_size, :] = (
                pred_features  # Store visual features
            )
...
def run_meg_individual_inference(config):
    ...
    files = [
        name
        for name in os.listdir(config["model_save_folder"])
    ...
    participant_ids = [int(name.split("_p")[1].split(".pth")[0]) for name in files]
    ...
    for p_id in sorted(participant_ids):
        ...
        if config["load_pretrained"]:
            model_path = (
                f"{config['model_save_folder']}/cliphba_dynamic_individual_cichy_p{p_id}.pth"
            )
