...
def run_image(
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_image_names, batch_images) in progress_bar:
            ...
            pred_embs = (
                pred_embs.cpu().numpy()
            )  # shape: (n_inference_timepoints, batch_size, n_dim)

            # Get the size of the current batch
            batch_size = batch_images.size(0)

            # Store embeddings and RDMs in the output arrays
            output_embs[:, current_idx : current_idx + batch_size, :] = (
                pred_embs  # Store embeddings
            )
        ...
        output_embs = (
            (output_embs - np.min(output_embs)) / (np.max(output_embs) - np.min(output_embs)) * 100
        )
        ...
        for i in range(n_inference_timepoints):
            ...
            hba_embedding = hba_embedding[
                ["image"] + [col for col in hba_embedding if col != "image"]
            ]
        ...
        np.save(
            f"{save_folder}/embeddings_{inference_start}ms-{inference_end}ms-{inference_step}step.npy",
            output_embs,
        )
        np.save(
            f"{save_folder}/rdms_{inference_start}ms-{inference_end}ms-{inference_step}step.npy",
            output_rdms,
        )
...
def run_meg_group_inference(config):
    ...
    list(range(config["inference_start"], config["inference_end"] + 1, config["inference_step"]))

    # Save model matrices
    np.save(
        f"{config['save_folder']}/weighting_matrix.npy", np.array(model.clip_model.weighting_matrix)
    )
