...
def main() -> None:
    ...
    parser.add_argument(
        "--dropout_rate", type=float, default=float(os.environ.get("DROPOUT_RATE", 0.3))
    )
    parser.add_argument("--lr", type=float, default=float(os.environ.get("LR", 1e-4)))
    parser.add_argument(
        "--weight_decay", type=float, default=float(os.environ.get("WEIGHT_DECAY", 1e-3))
    )
    parser.add_argument("--batch_size", type=int, default=int(os.environ.get("BATCH_SIZE", 64)))
    parser.add_argument("--epochs", type=int, default=300)
    parser.add_argument("--early_stopping_patience", type=int, default=20)
    parser.add_argument("--seed", type=int, default=int(os.environ.get("RANDOM_SEED", 1)))
    # --- Paths ---
    parser.add_argument(
        "--data_dir",
    ...
    parser.add_argument(
        "--preds_dir", default="./preds/meg_mem/", help="Directory for per-epoch prediction CSVs."
    )
