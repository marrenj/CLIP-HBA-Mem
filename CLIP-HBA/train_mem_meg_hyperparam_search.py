...
if TRAINING_DATA == "lamem":
    ...
    _EMBEDDINGS_DIR_BASE: str = (
        os.environ.get("MEG_EMBEDDINGS_DIR") or f"{_DATA_DIR}/combined_lamem_memcat/meg_embeddings/"
    )
...
def run_meg_sweep(
    ...
    base_sweep_dir = os.path.dirname(resume_db) if resume_db else f"./sweep_meg_out/{timestamp}"
    ...
    sweep_stdout.write("\n" + "=" * 80 + "\n")
    sweep_stdout.write(
        f"Search complete for tp={timepoint_ms:+d} ms. Full results: {results_csv}\n"
    )
    header = (
