...
def build_standardised_csv(dataset, output_path, data_dir="./Data", **kwargs):
    ...
    if dataset == "lamem":
        ...
        df_out = pd.DataFrame(
            {
                "image_path": df.apply(
                    lambda r: os.path.join(r["category"], r["subcategory"], r["image_file"]), axis=1
                ),
        ...
        fold = kwargs["fold"]
        csv_path = f"{data_dir}/combined_lamem_memcat/lamem_memcat_test_split_{fold:02d}.csv"
        img_root = {
...
def run_inference(config):
    ...
    for fold in folds:
        ...
        loader = DataLoader(
            ds, batch_size=config["batch_size"], shuffle=False, num_workers=4, pin_memory=True
        )
            ...
            if model_type in ("clip_hba_mem", "clip_frozen_mlp"):
                ...
                state_dict = {
                    (
                        ...
                        if (parts := k.split("."))[0] in _fc_to_idx
                        else k
                    ): v
        ...
        print(
            f"[Results] Pred range: [{all_preds.min():.4f}, {all_preds.max():.4f}]  "
...
def main():
    ...
    parser.add_argument(
        "--backbone", default="ViT-L/14", help="CLIP backbone name (clip_hba_mem only)."
    )
    parser.add_argument(
        "--vision_layers", type=int, default=2, help="DoRA vision layers (clip_hba_mem only)."
    )
    parser.add_argument(
        "--transformer_layers",
        type=int,
    ...
    parser.add_argument("--rank", type=int, default=32, help="DoRA rank (clip_hba_mem only).")
    ...
    parser.add_argument(
        "--output_dir", default="./preds/inference/", help="Directory to save prediction CSVs."
    )
